...
module.exports = class ClosureReducer extends Closure {
	...
	reduce(index, fact, context) {
		...
		return util.nowOrThen(this.closures[index].process(fact, context), fact => {
			if (this.options.matchOnce && context.currentRuleFlowActivated) {
				return fact;
			}
			const reduceStrategy = reduceStrategies[this.options.strategy] || reduceStrategies.last;
			return reduceStrategy(fact, this.reduce(index + 1, fact, context));
		});
	}
};
