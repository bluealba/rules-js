...
module.exports = class ClosureRegistry {
	...
	add(closure, override) {
		if (this.namedClosures[closure.name] && !override) {
			throw new Error(`Already defined a closure with name '${closure.name}'`);
		}
	...
	addProvidedClosureImpl(name, implementation, options) {
		...
		const closure = new ProvidedClosure(name, implementation, options);
		return this.add(closure);
	}
