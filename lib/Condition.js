 ...
 *
 * const condition = require("rules-js/Condition.decorator")
 *
 * @condition({
 * 	conditionName: "Side",
...
module.exports = class Condition {
	...
	evaluate(fact) { // eslint-disable-line no-unused-vars
		...
		throw new Error(`The rule ${name} is not defining any evaluation logic.
      Did you forget to implement the method evaluate(fact, params) in your
      rule class?`);
	}

	/**
	 * Creates a new instance of the Condition from the parsed json definition.
	 * The type of the condition must be determined in before-hand (we shouln't
	 * call Condition.instantiate(definition) but ConcreteCondition.instantiate(definition)
	 * instead)
	 */
