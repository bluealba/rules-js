...
module.exports = class ClosureRegistry {
	...
	_createRule(definition) {
		...
		const condition = this.parse(definition.when, { strategy: definition.conditionStrategy });
		const action = this.parse(definition.then);
		return new Rule(definition.name, condition, action, definition.conditionStrategy);
	}
