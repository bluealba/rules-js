...
const FunctionalClosure = require("./FunctionalClosure"),
	...
	LogicalClosure = require("./LogicalClosure"),
...
module.exports = class ClosureRegistry {
	...
	_createReducer(definition) {
		...
		return new ClosureReducer(definition.name, closures);
	}
	...
	_createRule(definition) {
		...
		let condition = this.parse(definition.when);
		if (definition.start && definition.end) {
			const dateRangeCondition = this.parse({ closure: "daterange", start: definition.start, end: definition.end });
			condition = new LogicalClosure(definition.name, "AND" , dateRangeCondition, condition);
		}
